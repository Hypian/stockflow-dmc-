...
async function migrateLegacyLocalStorage() {
  const lsProducts = LS.get('sf_products_final', []);
  if (lsProducts && lsProducts.length > 0) {
    try {
      console.log('Migrating', lsProducts.length, 'products from localStorage to PostgreSQL...');
      for (const p of lsProducts) {
        await API.saveProduct(null, { name: p.name, unit: p.unit, unit_price: p.unitPrice, active: p.active !== false });
      }
      
      const migratedProducts = await API.getProducts();
      const lsEntries = LS.get('sf_entries_final', []);
      
      if (lsEntries && lsEntries.length > 0) {
        console.log('Migrating', lsEntries.length, 'entries from localStorage...');
        const idMap = {};
        lsProducts.forEach(oldP => {
          const newP = migratedProducts.find(np => np.name === oldP.name);
          if (newP) idMap[oldP.id] = newP.id;
        });
        
        for (const e of lsEntries) {
          const newProdId = idMap[e.productId];
          if (newProdId) {
        ...
        LS.remove('sf_entries_final');
      }
      LS.remove('sf_products_final');
  ...
  content.innerHTML = `
    <div class="print-section" style="font-family:'Sora',Arial,sans-serif; max-width:1000px; margin:0 auto; color:#111;">
      ...
      <div style="margin-top:80px;text-align:center;font-size:8pt;color:#94a3b8;border-top:1px solid #e2e8f0;padding-top:20px;">
        ...
        <br/><span style="font-weight:700;color:#64748b;letter-spacing:0.05em;text-transform:uppercase;font-size:7pt;margin-top:4px;display:block;">StockFlow is a product designed and developed by Kagingo Brian</span>
      </div>
    </div>`;
...
function printShiftReport(today) {
  ...
  area.innerHTML = `
    <div class="print-section" style="font-family:'Sora',Arial,sans-serif; max-width:1000px; margin:0 auto; color:#111;">
      ...
      <div style="margin-top:80px;text-align:center;font-size:8pt;color:#94a3b8;border-top:1px solid #e2e8f0;padding-top:20px;">
        This is an official document generated by StockFlow Inventory System. Path: Reports/Shift/${getWorkingDate().replace(/-/g, '/')}/${currentUser.id}
        <br/><span style="font-weight:500; font-size:7.5pt; display:inline-block; margin-top:5px;">Designed and developed by Kagingo Brian</span>
      </div>
    </div>`;
  window.print();
}
  ...
  let html = `
    <div style="font-family: 'Sora', system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif; color: #0f172a; padding: 22px;">
      ...
      <div style="display:flex;justify-content:space-between;gap:12px;font-size:9px;color:#64748b;margin-top:18px;border-top:1px solid #e2e8f0;padding-top:8px;">
        <div>This document is system-generated by StockFlow. <span style="font-weight:700;color:#475569;">StockFlow is a product designed and developed by Kagingo Brian.</span></div>
        <div style="text-align:right;">&copy; ${new Date().getFullYear()} StockFlow</div>
      </div>
    </div>
  `;
